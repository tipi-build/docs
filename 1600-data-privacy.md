...
tipi.build is a SaaS deployment of CMake RE L1 remote execution.
...
* CMake RE Access Point & Dashboard is a set of Containerized micro-services
  - `api/build` : Deploys and manages developer machines digital twins
...
* CMake RE L1, L2 Build Caches are use S3 (or equivalent blob storage) to store produced build artifacts
...
*	Deployed with amazon-ecs-cli as _Amazon Elastic Container Service (ECS)_ on its own ECS Cluster
...
The CMake RE web platform is a fully custom implementation and does not rely on 3rd party services. The data logged is anonymizedm aggregated and is used only for short-term debugging purposes. 

Essentially CMake RE services can run fully isolated from any outside internet services.

## Authentication 
External OIDC providers are used to authenticate users, and CMake RE Dashboard only asks access to the user name and email. This information is stored in the service database along with a generated unique identifier to authenticate actions on the service.

These OIDC providers can be an internal self-hosted service or an external cloud service like Google, Github, Gitlab. 

Out of this identification managed by `api/accounts`, JSON Web Tokens are issued with asymmetric cryptographic signatures to authorize the users on the different services that make up the solution.
...
A typical user interaction with CMake RE consists of three main steps:

1. Dashboard Login and Identity Creation
2. Local Developer Machine Pairing
3. Remote Build Execution

All communications are secured either with TLS or SSH and users are authenticated with JWT Tokens and local-machine generated Private + Public Key pairs.

See more hereafter in the Sequence diagram.

#### OIDC and JWTs
The identity and authentication is essentially managed via an externally provided OIDC Identity Provider (OpenID Connect) to create an account in the CMake RE Dashboard via `api/accounts`.
 `api/accounts` then yields multiples JWTs (JSON Web Token) that are always by pairs of access and refresh token.
  - One set of JWTs authenticates to CMake RE L1 Remote Execution and L1 Build Cache
  - Another set of JWTs authenticates to the CMake RE L2 Remote Execution and L2 Build Cache
...
Cache visibility and access rights are by default private per user. Nothing is shared with any other users but can be configured to be shared by repositories and groups of users.
...
A developer-pc digital twin is a short-lived instance to run builds remotely ( _i.e._ A freshly provisioned virtual machine started on demand in the customer's private cloud account ).
...
Short lived digital twins get destroyed at the end of the build session and cleaned up after a configurable timeout.

The developer-pc digital twin remote build runner is made accessible only through asymmetric cryptographic keys for the SSH Protocol generated on each new build session, which means that only the local machine sending the source code and initiating the session has the data to access the short-lived remote build machines and the decrypted sources.

The tokens to the source code hosting solution are exchanged from the Local Machine to the digital-twin via cmake-re encrypted vault technology over TLS. For details read the next section.

## Secure access token storage 
On the first registration CMake RE users are asked to create a vault. A vault is a cryptographically secure container that stores access tokens to user's private repositories hosted elsewhere ( e.g. Internal Gerrit, Gitlan, Github… ).
...
No serious attacker would actually choose this road, as the encrypted token after such a data breach could just be revoked.
